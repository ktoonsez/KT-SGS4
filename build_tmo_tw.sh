...
curdate=`date "+%m.%d.%Y"`
ktver="--KT-SGS4-TW-JB-TMO-"$curdate"--"
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	FILENAME=../KT-SGS4-TW-JB-TMO-$curdate.zip
	FILESIZE=$(stat -c%s "$FILENAME")
	echo "Size of $FILENAME = $FILESIZE bytes."
	rm ../TMOversion.txt
	exec >>../TMOversion.txt 2>&1
	echo "KT-SGS4-TW-JB-TMO-"$curdate", "$FILESIZE", http://ktoonsez.jonathanjsimon.com/sgs4/tw/KT-SGS4-TW-JB-TMO-"$curdate".zip"
