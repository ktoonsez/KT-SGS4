...
export INITRAMFS_SOURCE=`readlink -f ..`/Ramdisks/TW_INTL
...
curdate=`date "+%m.%d.%Y"`
ktver="--KT-SGS4-TW-JB-INTL-"$curdate"--"
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm ramdisk.gz
	rm zImage
	rm ../KT-SGS4-TW-JB-INTL*.zip
	zip -r ../KT-SGS4-TW-JB-INTL-$curdate.zip .

	time_end=$(date +%s.%N)
	echo -e "${BLDYLW}Total time elapsed: ${TCTCLR}${TXTGRN}$(echo "($time_end - $time_start) / 60"|bc ) ${TXTYLW}minutes${TXTGRN} ($(echo "$time_end - $time_start"|bc ) ${TXTYLW}seconds) ${TXTCLR}"

	FILENAME=../KT-SGS4-TW-JB-INTL-$curdate.zip
	FILESIZE=$(stat -c%s "$FILENAME")
	echo "Size of $FILENAME = $FILESIZE bytes."
	rm ../INTLversion.txt
	exec >>../INTLversion.txt 2>&1
	echo "KT-SGS4-TW-JB-INTL-"$curdate", "$FILESIZE", http://ktoonsez.jonathanjsimon.com/sgs4/tw/KT-SGS4-TW-JB-INTL-"$curdate".zip"
