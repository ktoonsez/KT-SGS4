...
curdate=`date "+%m.%d.%Y"`
ktver="--KT-SGS4-AOSP-JB-TMO-"$curdate"--"
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm ramdisk.gz
	rm zImage
	rm ../KT-SGS4-AOSP-JB-TMO*.zip
	zip -r ../KT-SGS4-AOSP-JB-TMO-$curdate.zip .

	time_end=$(date +%s.%N)
	echo -e "${BLDYLW}Total time elapsed: ${TCTCLR}${TXTGRN}$(echo "($time_end - $time_start) / 60"|bc ) ${TXTYLW}minutes${TXTGRN} ($(echo "$time_end - $time_start"|bc ) ${TXTYLW}seconds) ${TXTCLR}"

	FILENAME=../KT-SGS4-AOSP-JB-TMO-$curdate.zip
	FILESIZE=$(stat -c%s "$FILENAME")
	echo "Size of $FILENAME = $FILESIZE bytes."
	rm ../TMOversion.txt
	exec >>../TMOversion.txt 2>&1
	echo "KT-SGS4-AOSP-JB-TMO-"$curdate", "$FILESIZE", http://ktoonsez.jonathanjsimon.com/sgs4/aosp/KT-SGS4-AOSP-JB-TMO-"$curdate".zip"
