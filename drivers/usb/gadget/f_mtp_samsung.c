...
static struct usb_interface_descriptor ptp_interface_descS = {
	.bLength                = USB_DT_INTERFACE_SIZE,
	.bDescriptorType        = USB_DT_INTERFACE,
	.bInterfaceNumber       = 0,
	.bNumEndpoints          = 3,
...
static struct usb_descriptor_header *fs_ptp_descsS[] = {
	(struct usb_descriptor_header *) &ptp_interface_descS,
	(struct usb_descriptor_header *) &fs_mtpg_in_desc,
	(struct usb_descriptor_header *) &fs_mtpg_out_desc,
	(struct usb_descriptor_header *) &int_fs_notify_desc,
	NULL,
...
static struct usb_descriptor_header *hs_ptp_descsS[] = {
	(struct usb_descriptor_header *) &ptp_interface_descS,
	(struct usb_descriptor_header *) &hs_mtpg_in_desc,
	(struct usb_descriptor_header *) &hs_mtpg_out_desc,
	(struct usb_descriptor_header *) &int_hs_notify_desc,
...
static int mtp_ctrlrequestS(struct usb_composite_dev *cdev,
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (value >= 0) {
		...
		printk(KERN_DEBUG "mtp_ctrlrequestS "
...
static int mtp_bind_configS(struct usb_configuration *c, bool ptp_config)
{
	...
	/*Test the switch */
	if (ptp_config) {
		mtpg->function.descriptors = fs_ptp_descsS;
		mtpg->function.hs_descriptors = hs_ptp_descsS;
	} else {
