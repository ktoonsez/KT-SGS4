...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	./mkbootimg --cmdline 'androidboot.hardware=qcom user_debug=31 zcache msm_rtb.filter=0x3F ehci-hcd.park=3 androidboot.selinux=permissive' --kernel $PACKAGEDIR/zImage --ramdisk $PACKAGEDIR/ramdisk.gz --base 0x80200000 --pagesize 2048 --ramdisk_offset 0x02000000 --output $PACKAGEDIR/boot.img 
	#if [ $EXEC_LOKI = 'Y' ]; then
	#	echo "Executing loki"
