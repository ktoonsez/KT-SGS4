...
export INITRAMFS_SOURCE=`readlink -f ..`/Ramdisks/TW_VZW
...
curdate=`date "+%m.%d.%Y"`
ktver="--KT-SGS4-TW-JB-VZW-"$curdate"--"
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	FILENAME=../KT-SGS4-TW-JB-VZW-$curdate.zip
	FILESIZE=$(stat -c%s "$FILENAME")
	echo "Size of $FILENAME = $FILESIZE bytes."
	rm ../VZWversion.txt
	exec >>../VZWversion.txt 2>&1
	echo "KT-SGS4-TW-JB-VZW-"$curdate", "$FILESIZE", http://ktoonsez.jonathanjsimon.com/sgs4/tw/KT-SGS4-TW-JB-VZW-"$curdate".zip"
